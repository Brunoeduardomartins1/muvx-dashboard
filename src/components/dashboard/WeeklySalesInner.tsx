...
interface TooltipProps {
  active?: boolean
  payload?: Array<{ value: number; payload: WeekdaySales }>
  label?: string
}
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div style={{ background: '#18181B', border: '1px solid #2A2D35', borderRadius: 12, padding: '10px 16px' }}>
        <p style={{ fontSize: 10, color: '#6B7280', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>{d.day}</p>
        <p style={{ fontFamily: 'var(--font-space-grotesk)', fontWeight: 700, color: '#08F887', fontSize: 20 }}>{fmtNum(d.count)}</p>
        <p style={{ fontSize: 11, color: '#9CA3AF', marginTop: 2 }}>vendas</p>
      </div>
    )
  }
  return null
...
export default function WeeklySalesInner({ data }: { data: WeekdaySales[] }) {
